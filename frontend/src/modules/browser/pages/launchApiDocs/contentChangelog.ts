...
- 每实例代理网关：每个运行实例使用独立本地 SOCKS5 端口，控制界面退出后浏览器和网关继续运行并可恢复状态。
- 代理热切换：新连接立即使用新代理，旧连接默认自然排空；可选择立即断开旧连接，切换失败时保持原路由。
- 分流规则：支持全部代理、规则分流、全部直连，以及精确域名、域名后缀、关键词、IP/CIDR 的代理、直连、阻断动作。
- 防流量逃逸：代理连接失败时直接失败，不自动回退直连。
- 公共 API：新增代理切换、网关状态和分流配置接口，并补齐脚本公开 Hook 等现有接口文档。

## 2.1.0 - 2026-08-18

- 同步正式版：加入指纹检测、HTTPS 代理桥接、会话恢复、实例内存限制、删除审计和启动服务设置。
- 保留个人版：树形实例/代理分组、订阅 User-Agent、分组 API 和代理兼容策略保持不变。
- 数据库迁移：在个人版 v13/v14 后追加正式版字段迁移，已有数据无需重建。

## 2.0.1 - 2026-07-14

- 分组 API：新增实例树分组和代理/订阅树分组的查询、创建、更新、删除与批量移动 HTTP 接口。
- API 文档：仓库和应用内文档同步说明分组接口、鉴权方式、事务边界与升级兼容性。

从 2.0.0 升级到 2.0.1 不需要新的数据库迁移；现有实例、代理、分组、订阅和自动化数据会继续保留。
