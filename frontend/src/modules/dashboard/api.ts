...
export async function fetchDashboardStats(): Promise<DashboardStats> {
  ...
  if (bindings?.GetDashboardStats) {
    try {
      ...
      const licenseStatus = bindings.GetLicenseStatus ? await bindings.GetLicenseStatus() : { maxLimit: 9999 }
      return {
        ...
        maxProfileLimit: licenseStatus?.maxLimit ?? 9999,
  ...
  return { totalInstances: 0, runningInstances: 0, proxyCount: 0, coreCount: 0, memUsedMB: 0, maxProfileLimit: 9999, appVersion: 'unknown' }
}
