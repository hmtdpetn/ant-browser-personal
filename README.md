...
- 为每个运行中的浏览器实例提供独立的本地 SOCKS5 代理网关，支持运行中热切换代理。
- 新代理连接立即使用新路由，旧连接可自然排空或使用 `force` 强制断开；代理失败时不会静默回退直连。
- 新增 `PROXY`、`DIRECT`、`BLOCK` 分流动作，以及精确域名、域名后缀、域名关键词和 IP/CIDR 匹配规则。
- 规则按从上到下、第一条命中执行；规则分流未命中时默认走代理。
- 主界面和实例编辑页增加代理网关状态、分流规则和热切换操作。
- 新增代理网关公共 HTTP API，并在应用内 API 文档中补充参数、请求示例、响应和错误码。
- 数据库迁移只增加代理分流配置，不覆盖已有实例、代理、用户数据或指纹参数。

### 2.1.1 · 2026-08-18

- 实例分组改为逐层浏览：父分组只列出直属实例，子分组以紧凑文件夹显示，点击后再进入下一层。
- 增加当前分组路径和返回上级入口；长文件夹名会自动截断，避免挤压实例操作区域或产生额外横向滚动。

### 2.1.0 · 2026-08-18

- 同步正式版最新主线：加入指纹检测与能力矩阵、HTTPS 代理桥接、SOCKS5/链式代理修复、实例删除审计、会话恢复、内存限制和启动服务设置。
- 保留个人版能力：实例/代理树形分组、订阅 User-Agent 策略、分组 Launch HTTP API、任务栏实例标识、个人代理兼容和可折叠侧栏继续有效。
- 数据库按个人版迁移顺序追加正式版字段，升级时不会覆盖已有实例、代理、订阅和分组数据。

### 2.0.1 · 2026-07-14

- Launch HTTP API：新增实例树分组与代理/订阅树分组 API，支持列表、新建、更新、删除以及批量移动成员；所有接口继承本地监听和 API Key 鉴权规则。
- API 文档：新增 [API.md](API.md)，补齐既有健康检查、实例、启动、运行状态、自动化和 CDP 入口说明，并明确哪些功能仅由桌面端处理。

### 2.0.0 · 2026-07-14

- 订阅 UA 策略：Clash 订阅支持 Ant / FlClash 预设与自定义 User-Agent，可在失败后按安全顺序自动重试，刷新订阅时会沿用保存的策略。
- 树形分组：实例与代理均支持持久化主分组/子分组、父分组聚合筛选和批量移动；删除代理分组时会将代理和子分组提升到上一级。
- 数据升级：数据库迁移新增订阅 UA 和代理分组字段，旧版按名称保存的代理分组会自动迁移；备份合并兼容旧版和新版代理表结构。
- 图标与界面：实例图标升级为多尺寸高清 ICO，分组侧栏可折叠，分组弹窗提升到页面顶层；实例表格支持仅横向调整列宽并记忆设置。
- 兼容性修复：订阅刷新会保留代理所属的树形分组 ID，避免同名子分组被错误移动到根级。
...
- 每实例代理网关：支持运行中热切换、连接排空和 PROXY / DIRECT / BLOCK 分流
